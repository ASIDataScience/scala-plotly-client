...
case class CartesianPlot(
  ...
  def withScatter[X: Writable, Y: Writable](
    ...
    val xsAsPType = xs.map {
      implicitly[Writable[X]].toPType
    }
    val ysAsPType = ys.map {
      implicitly[Writable[Y]].toPType
    }
...
case class ThreeDPlot(
  ...
  def withScatter[X: Writable, Y: Writable, Z: Writable](
    ...
    val xsAsPType: Iterable[PType] = xs.map {
      implicitly[Writable[X]].toPType
    }
    val ysAsPType: Iterable[PType] = ys.map {
      implicitly[Writable[Y]].toPType
    }
    val zsAsPType: Iterable[PType] = zs.map {
      implicitly[Writable[Z]].toPType
    }
